...
import {
  ...
  resetPassword,
  fetchAllCategories,
  getSlotsByWorkerIdService,
  getUserBookedWorkers,
...
export const googleLoginHandler = async (req: Request, res: Response) => {
  try {
    ...
    const imageBuffer = await sharp(imageResponse.data).jpeg().toBuffer();
...
export const updateProfile = async (req: any, res: Response) => {
  try {
    ...
    if (profileImage) {
      cloudinary.uploader
        .upload_stream(async (error: any, result: any) => {
          if (error) {
            return res
              .status(500)
              .json({ error: "Failed to upload image to Cloudinary" });
          ...
          if (newPassword) {
            ...
            if (!isMatch) {
              return res.status(400).json({
                error:
                  "Old password is incorrect. Please use the forgot password option.",
              });
            }
...
export const getSlotsByWorkerIdController = async (
  req: Request,
  res: Response
) => {
  try {
...
export const getUserBookedWorkersController = async (
  req: any,
  res: Response
) => {
  try {
